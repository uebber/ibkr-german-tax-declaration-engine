...
**Account-agnostic (merged) FIFO — known limitation.** FIFO ledgers are keyed by security (and by currency for FX), merged across the taxpayer's accounts, rather than per Depot. This is correct for single-account holdings and inter-account transfers, and is imprecise only when the same security or foreign currency is co-held in two accounts simultaneously and disposed from one. Security co-holding is detected and warned per-snapshot; currency co-holding (e.g. USD across two accounts) is surfaced as a single summary warning. See `reference/tax-law/estg-20-kapitalvermoegen.md` (Abs. 4). A full per-Depot FIFO is a deferred change.
