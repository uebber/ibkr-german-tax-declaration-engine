...
**Per-Depot FIFO and internal transfers.** FIFO ledgers are keyed by `(custody account, asset)` — for securities and foreign currency (FX) alike — so FIFO is applied per Depot (§20 Abs. 4 S. 7 EStG); a disposal from one account consumes only that account's lots. Events carry their IBKR `ClientAccountID`; per-account SoY/EoY positions and cash balances drive the reconstruction, while the cross-account aggregate is used for Vorabpauschale (per person), EoY validation, and the tax-return totals. Inputs without an account id collapse to a single default ledger (unchanged single-account behaviour). Internal transfers between the taxpayer's own accounts are parsed from the IBKR *Transfers* export and modelled as tax-neutral `InternalTransferEvent`s (Fußstapfentheorie, §43 Abs. 1 S. 5): the FIFO lots carry over unchanged — for a long position the acquisition date and EUR cost basis, for a transferred short position the opening date and EUR sale proceeds — realising no gain. The historical SoY reconstruction replays each account's trades and the inter-account transfers in a single chronological stream, so a security bought, transferred between Depots and (partly) sold all within the historical window is rebuilt lot-exactly. See `reference/tax-law/estg-20-kapitalvermoegen.md` (Abs. 4).
...
- **Bond Maturity (BM):** Redemption of a bond at maturity (Fälligkeit). Under §20 Abs. 2 Satz 1 Nr. 7 EStG the Einlösung is treated as a Veräußerung. There is no IBKR trade record for the maturity itself — only a corporate action record with `Type="BM"`. The parser maps it to a synthetic `TradeEvent` of type `TRADE_SELL_LONG` (proceeds = the CA record's total `Proceeds`, price per bond = proceeds / quantity, no commission), so it reuses the bond FIFO and currency-impact path and is replayed historically like any other trade. Realized G/L (`RealizationType.LONG_POSITION_SALE`) is reported in Anlage KAP as Other Capital Income: positive → Zeile 19 (`kap_other_income_positive`), negative → Zeile 22 (`kap_other_losses_abs`), consistent with bond sales.
...
- **§19 Abs. 1 S. 3-4 InvStG — Vorabpauschale deduction at disposal:** the gross gain is reduced by the gross (pre-Teilfreistellung) Vorabpauschalen assessed during the holding period of the sold units, computed **per lot, FIFO** (`per-unit VP[year] = declared_VP[year] / units_held_at_year_end`; a full exit reduces to `Σ declared_VP[year]`). The deduction is **capped at the VP the user declared** in prior years (Z9-13) — prompted once and cached via `DeclaredVpProvider` (`cache/declared_vp.json`), resolved for funds disposed in the tax year. Undeclared years are not deducted (warn-only; gain left unreduced). The deducted amount is stored on `RealizedGainLoss.vp_deduction_eur`.
- **For reporting on Anlage KAP-INV (Zeilen 14, 17, 20, 23, 26), the GROSS Gain/Loss is used** — i.e. **net of the §19 VP deduction but gross of Teilfreistellung** (Aufstellung mode; the VP deduction belongs to Zeile 53 of the per-sale Ermittlung, not a separate summary line; form Zeile 55 = bestandsgeschützte Alt-Anteile, unrelated).
...
- **Per §18 InvStG**, the Vorabpauschale is computed for every `InvestmentFund` held at year-end: `Basisertrag = year-start NAV per unit × units held × 0.70 × Basiszins`, then `VP = max(0, Basisertrag − distributions)` capped at the value gain, then Teilfreistellung. The Basiszins comes from `config.BASISZINS_BY_YEAR`.
- **Partial-year acquisitions (§18 Abs. 2 Satz 2):** for units acquired during the tax year, the VP is reduced by 1/12 per full month preceding the acquisition month. The engine applies a units-weighted `(13 − M)/12` factor derived per FIFO lot (multi-tranche). The required **year-start (1 Jan) NAV** of a fund not held on 1 Jan is absent from IBKR data; it is collected interactively and cached (`FundSoyNavProvider`, cache `cache/fund_soy_nav.json`, year-specific key), resolved by a pre-pass in `pipeline_runner` (`processing/vp_nav_resolution.py`).
- **Deemed inflow / assessment year (§18 Abs. 3):** the VP computed for calendar year Y is deemed to flow on the first business day of Y+1 and is declared in the **Y+1** assessment. `VorabpauschaleData.deemed_inflow_year = Y + 1`; the KAP-INV lines (Z9–13, Z55) for a run of tax year X are fed by the VP with `deemed_inflow_year == X` (i.e. the **X−1** VP). A run of year X therefore computes both the X−1 VP (this year's return lines) and the X VP (shown as a "VZ X+1" preview). The X−1 VP needs the prior-year start values: the prior-year SoY positions export (`Positions-{X-1}-SoY.csv`, loaded as `positions_prior_start`) for funds held on 1 Jan X−1, and interactive NAVs for funds acquired during X−1.
- **Missing-data handling (warn-only):** any start-of-year NAV that cannot be resolved — current or prior year, manual NAV or the prior-year positions export — is recorded as a `VorabpauschaleGap` and surfaced as an explicit callout in the report's Vorabpauschale section. Runs are never aborted for missing VP inputs.
