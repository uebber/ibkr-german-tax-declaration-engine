...
| `AssetClass`       | `asset_class`               | `str`               | Asset class. `CASH` rows produce no event.               | Required. Example: "STK", "CASH"                                                                             |
...
**Notes:**
- **`TransferPrice` and `CostBasis` are parsed but never consumed.** They *are* a cost basis, but in IBKR's convention: an option-assignment premium is netted into the basis of the assigned shares, which German law rejects ([GT-ESTG20-004], BMF 14.05.2025 Rz. 26). The carried basis is the sending account's reconstruction (with separate option-treatment limitations recorded in the legal implementation map), so the lots relocate with *that* basis and these two values are read to satisfy the required-column contract, not to compute or check a figure. What the `LOT` rows are used for -- the acquisition day, the quantity, and the long/short sign -- is matched and cross-checked against the sending ledger.
- **`TransactionID` is not carried onto the move event.** The two sides carry different ids, so neither names the move, and an id would decide the intra-day order (which the engine fixes by rule — a move sorts before that day's trades so units that arrived can be sold).
