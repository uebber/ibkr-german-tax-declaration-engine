...
def download_and_merge(
    ...
    prefix_mapping = {
        ...
        # A query type missing here is not skipped loudly: the loop below reaches
        # `prefix_mapping.get(query_type)`, finds None, and `continue`s without a word.
        # Transfers was absent while its query ID was configurable, so a `--download` run
        # silently produced no Transfers file for the year -- which the engine then reads
        # as a year it cannot see a move in.
        "transfers": "Transfers",
    }
    ...
    for query_type, query_id in query_ids.items():
        ...
        if prefix is None:
            ...
            # missing: a query type the user has configured an ID for is one they expect
            # to be downloaded, and skipping it without a word produces a `data_import/`
