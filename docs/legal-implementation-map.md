...
| GT-ESTG20-038 | implements | `src/classification/asset_classifier.py` `preliminary_classify` — `CFD`, `FXCFD`, `CMDTY`, `OPT`, `FUT` → derivative categories | `test_asset_classification_derivatives.py` | Rz. 9's definition is what lets an IBKR asset class be mapped to a Termingeschaeft without an analogy. The two exclusions it states (Zertifikate, Optionsscheine) have no IBKR asset class of their own and are not distinguished — a Zertifikat arrives as `STK` and is treated as a share. Recorded as a known limit. |
