...
a single-account export uses one normalized account key. Currency ledgers use their
explicit pooled boundary, independent of the presence of that account label. Currency (foreign-cash) ledgers remain pooled per person — that is a separate concern
...
long/short disposals, multiple affected holdings and merger provenance. This
