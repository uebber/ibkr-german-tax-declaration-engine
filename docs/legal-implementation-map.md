...
| GT-ESTG20-066 | implements | `src/processing/enrichment.py` (folds the commission **and** the trade `Taxes` into the buy's EUR cost basis `net_proceeds_or_cost_basis_eur`, for current-year and historical trades alike); the currency the buy consumes is drawn by `src/engine/event_processors/trade_processor.py` (`_process_transaction_tax_currency_impact`) for a tax-year trade and by the historical currency replay in `src/engine/calculation_engine.py` for a prior-year one; `src/parsers/raw_models.py`, `src/parsers/domain_event_factory.py`, `src/domain/events.py` carry the tax onto the trade | `test_transaction_taxes.py`, `test_precision.py` | Anschaffungskosten = purchase price + Anschaffungsnebenkosten (§ 255 Abs. 1 S. 2 HGB): a broker commission and a purchase transaction tax (UK/HK stamp duty) both raise the acquired lot's cost basis and the foreign currency the buy consumed. The tax is in the trade's own currency (no separate tax-currency column). A non-zero tax on a **sale** raises (`DataIntegrityError`); none occurs in the data. This claim also grounds the commission capitalisation, which previously had no row. |
...
| GT-ESTG20-013 | implements | `src/engine/calculation_engine.py` (one FIFO ledger per `(account, asset)`; the historical replay, the tax-year dispatch, the mark reconciliation and the end-of-year check all keyed by account), `src/engine/ledger_views.py`, `src/utils/account_utils.py`, `src/domain/events.py` (`FinancialEvent.account_id`), `src/parsers/domain_event_factory.py` (populated from `ClientAccountID`) | `test_per_account_fifo.py`, `test_ledger_views.py`, `test_multi_account_harness.py` | See below for Q2's chosen reading and what this does **not** yet cover. |
| GT-ESTG20-061 | implements | `src/parsers/parsing_orchestrator.py` — every account's row recorded, for the opening and closing snapshots, the checkpoint marks, the preceding year's Vorabpauschale snapshots and the cash-balance report; `src/domain/assets.py` — `person_snapshot()` sums them into the person's holding | `test_person_level_snapshot.py` — the opening read, the closing read, a checkpoint mark, the preceding year's three snapshots, a currency held in two accounts, a fund held in two accounts reaching Zeilen 9–13, and the refusal to add two currencies | The taxable subject is the person (§ 2 Abs. 1 S. 1 Nr. 5, § 25 Abs. 1 EStG), so what is declared is the total across that person’s accounts. A Flex Query covering several accounts emits one snapshot row per account; each row was assigned in turn, so the record that survived was whichever the file ended with — one account's holding, presented as the whole. Every snapshot is now recorded under the account that reported it, so the rows cannot overwrite each other, and the person's figure is `person_snapshot()` / `person_mark()` derived from them rather than stored. A blank column stays `None` across the derivation, so the missing-cost-basis guard still fires. **Which lot a disposal consumes** where one security sits in two accounts is not this claim but [GT-ESTG20-013], now implemented: ledgers are keyed per account and a disposal consumes its own account's lots, while the person's total for this claim is `person_snapshot()` over the account rows. **For a currency balance, what the store settles is narrower and is worth stating exactly.** [GT-FX-008] gives Rz. 131 verbatim -- FIFO over *gleichartige Fremdwaehrungsbetraege* -- and records that § 20 Abs. 4 Satz 7's *einzelnes Depot* condition cannot reach a Fremdwaehrungsguthaben at all, which is neither a Wertpapier nor in Sammelverwahrung. So the Depot boundary that Rz. 97 imposes on securities has **no source that carries it over** to a currency balance; that the pool therefore crosses accounts is an inference from the absence of a boundary, not a sentence anyone wrote. **And what is open is a reading, not an unread source**: Rz. 131 is in `reference/` verbatim and in full, retrieved and read on 2026-08-03, and the FIFO sentence above is the last sentence of its **second** paragraph. What that paragraph also says is that moving a balance *"auf ein anderes verzinsliches Konto bei demselben oder einem anderen Kreditinstitut"* is a disposal of the original Kapitalforderung and an acquisition of a new one -- which bears directly on whether each account's balance is its own. **That is now settled: [GT-FX-009] reads each account's balance as its own Kapitalforderung and [GT-FX-010] values a move between two of them (Reading A), and the currency ledgers are keyed per account accordingly** — the store claim was written and verified before the code, as the Ground Truth Rule requires. Incidence is in `VALIDATION_REPORT.md`. |
| GT-ESTG20-014 | implements | `src/parsers/transfers_parser.py` (reads the Transfers export strictly), `src/parsers/domain_event_factory.py` (`create_events_from_transfers` — collapses the summary rows into one move per side pair and reads the `LOT` rows as the per-acquisition-day breakdown), `src/engine/event_processors/transfer_processor.py` (`apply_internal_transfer` — relocates the lot objects), `src/engine/calculation_engine.py` (the move is a `Phase.LEDGER_EVENTS` stream item in the historical replay and an `InternalTransferProcessor` in the tax year, plus the counterparty and complete-window guards), `src/utils/sorting_utils.py` (the move's intra-day band) | `test_internal_transfers.py`, `test_transfers_parser.py` | An own-account move is not a Veräußerung under Abs. 2: no change of beneficial owner and no consideration, so the lots **relocate** between the two accounts' ledgers rather than close and reopen. The relocation moves the lot **objects**, so the acquisition date, the German cost basis, `acquisition_date_is_known` and the accumulated Vorabpauschale carry over by construction. The German basis is the sending ledger's own reconstruction: the export's `CostBasis`/`TransferPrice` **are** a cost basis but in IBKR's convention, which nets an option-assignment premium into the basis of assigned shares — contrary to [GT-ESTG20-004] — so they are parsed to complete the required lot-detail export shape and never consumed. What the `LOT` rows do drive is which lots moved: the acquisition day and quantity are matched against the sending ledger and the long/short sign is cross-checked against it. A move of some of the acquisition days but not all is now supported (matched day by day from the `LOT` rows); the one case that still stops the run is a move splitting a **single** acquisition day, where the export does not say which sub-lot went (zero measured incidence). A move naming an account the input does not otherwise report stops the run (`TRANSFER_COUNTERPARTY_UNKNOWN`) — `Type=INTERNAL` says the counterparty is an IBKR account, not that it is the taxpayer's own. Currency (cash) moves are a different question and out of this claim's scope: a currency balance is not a Wertpapier and a move of one is a disposal plus an acquisition, not a relocation — the store settles that each account's balance is its own Kapitalforderung at [GT-FX-009] and values the move at [GT-FX-010], and the engine now implements both (currency ledgers keyed per account; a cash Umbuchung read as `InternalCashTransferEvent`). See the [GT-ESTG20-013] block above. |
...
| GT-INVSTG-011 | implements | `abs2_retained_twelfths()` in `src/engine/vorabpauschale_attribution.py`, used by `_calculate_vorabpauschale()` and by the Zeile 53 distribution key; the tranche loop in `_calculate_vorabpauschale()` for units whose acquisition date the reconstruction could not recover | `test_vorabpauschale_abs2.py`, `test_vorabpauschale_zeile53.py::TestAbs2Twelfths`, `test_vorabpauschale.py::TestAFundAcquiredDuringTheYear` for the effect on a declared figure, and `test_vorabpauschale.py::TestUnitsTheReconstructionCouldNotDate` for units the report places before the year | **What the twelfths multiply is GT-INVSTG-056**; this row is how many of them are kept. Abs. 2 reduces each acquisition by one twelfth per full month before its month of acquisition; units held before the year keep twelve. **Units the reconstruction could not date reach that answer without a date being invented for them**, corrected 2026-08-31: Abs. 2 asks only whether a tranche was acquired *during* the year, and the preceding snapshot answers it for units the broker already reported at the close of the year before — the store settles the consequence at `invstg-18-vorabpauschale.md:131-135`. Above that reported count the question is unanswerable and the fund is refused. Superseded behaviour: the branch rewrote the tranche's date to 31 December of the preceding year and left `acquisition_date_is_known` false, so the next step **raised `ProcessingError` and stopped the run** for exactly the case the branch exists to let through; no test reached it, and it needed an undated lot and a prior-opening snapshot together. **No longer a choice under uncertainty** — the 2026-08-07 audit closed Q13 in favour of exactly this reading, on Rz. 18.11's worked example applying the reduction to the *per-Anteil* amount, with Rz. 18.9 and Rn. 184a alongside. The one residual is recorded in the store, not here: no source works a mixed holding, so summing per acquisition joins two quoted rules. Superseded text: *"implements (Q13 Reading A: per acquisition) … a choice under uncertainty"*; and before that, **Abs. 2 pro-rata was not implemented** at all. |
