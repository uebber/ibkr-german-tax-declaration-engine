...
| GT-ESTG20-013 | implements | `calculation_engine.py`, `ledger_views.py`, `account_utils.py`; `option_trade_linker.py` and `option_premiums.py` preserve delivery/premium ownership by account | `test_per_account_fifo.py`, `test_ledger_views.py`, `test_multi_account_harness.py`, `test_option_delivery_integrity.py` | Ledgers and premium allocations are account-scoped. Same-day option consumption retains chronology; repeated and partial deliveries cannot consume another account's premium. See Q2 below and the separate GT-ESTG20-004 treatment deviation. |
