...
unchanged. Currency (foreign-cash) ledgers remain pooled per person — a recorded deviation now that the store
settles each account's balance as its own Kapitalforderung ([GT-FX-009], valued at [GT-FX-010]),
closed by the per-account currency change; see those rows, the `GT-ESTG20-061` row and `reference/`.
...
synthesised. `MULTI_ACCOUNT_LIMITATIONS` accordingly drops the securities caveat when the export is
read; it stays only when no Transfers export was supplied (a move would then still be invisible), and
a supplied export missing a year of the window stops the run (`TRANSFERS_WINDOW_INCOMPLETE`). What is
not closed is the currency half: a foreign-currency balance is still pooled per person, so a cash
Umbuchung between accounts is inert — now a recorded deviation at [GT-FX-009]/[GT-FX-010], closed by
the per-account currency change.
...
| GT-FX-009 | **deviates** | — | — | One account, one Kapitalforderung. Every currency ledger is keyed `DEFAULT_ACCOUNT` in `src/engine/calculation_engine.py`, so a disposal from one account consumes amounts paid into another, and the cost basis it is measured against is the wrong one. Closed by the `feat-func` that keys currency ledgers by the real account. Incidence measured against the real export before the work — non-EUR currencies sit in more than one account in the two-account years; the counts are in `VALIDATION_REPORT.md`. |
| GT-FX-010 | **deviates** | — | — | What a move between two of the taxpayer's own currency accounts is worth. Nothing reads such a move today: the rows are parsed and deliberately dropped in `DomainEventFactory.create_events_from_transfers`, so no disposal is declared and no lot changes account. Closed by the same `feat-func`. **The closing change takes Reading A of Q15** — the Veräußerungspreis and the new lot's Anschaffungskosten are the gemeiner Wert of the Kapitalforderung received, i.e. the amount moved, converted per [GT-ESTG20-022]. Reason: Rz. 131 ¶2 makes the Umbuchung a Veräußerung in terms, so Reading B contradicts a Tier 2 sentence, which CLAUDE.md's grey-area rule bars whichever way it falls; and Rn. 65/66, 66a and 69 measure a non-money consideration at the value of what is received in every configuration the administration addresses. **It is an extension, not an application** — none of those Randziffern names a currency account, and that is why Q15 stays open rather than being retired. |
