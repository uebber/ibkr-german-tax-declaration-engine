...
| GT-ESTG20-062 | not reached | — | — | § 22 Nr. 3 Satz 1's subsidiarity clause and Satz 2's 256-Euro Freigrenze, with Satz 3 and Satz 4 on losses. No engine behaviour turns on it directly; it is the ground of GT-ESTG20-063, and it is what makes the receipt side of an award a matter for Anlage SO rather than Anlage KAP. The Freigrenze itself is out of scope, as GT-ESTG23-009. |
| GT-ESTG20-063 | **deviates** | `src/engine/event_processors/stock_award_processor.py` — records the undeclared receipt as a data gap; nothing declares it | `tests/test_stock_award_scenarios.py::test_a_vesting_in_the_tax_year_reports_the_receipt_it_does_not_declare`, and `::test_an_award_and_a_reversal_report_no_receipt` for the other two kinds | A benefit granted for placing capital is a Leistung under § 22 Nr. 3, not Kapitalertrag — the same three-step result the store reached for the Wertpapierdarlehen fee at GT-ESTG20-049, failing Nr. 7 on its element rather than its gate. **The engine does not declare the receipt**, because the reporting layer has no Anlage SO *Einnahmen aus Leistungen* category — only `ANLAGE_SO_Z54_NET_GV`, for § 23 — and `reference/tax-forms/anlage-so-zeilen.md` holds no Zeilen for the Leistungen half of the form. That is the same gap as **issue #76**, and it is that issue's to close for both payment kinds at once. Measured 2026-08-13 across `Grants-2022.csv`…`Grants-2025.csv`: award receipts occur in **VZ 2022, VZ 2023 and VZ 2024**, so a figure is undeclared under this claim in each. |
| GT-ESTG20-064 | **implements**, subject to the factual condition below | `src/parsers/domain_event_factory.py` (`create_events_from_grants` — a vesting is dated on `VestingDate`, not the broker's `ReportDate`); `src/processing/enrichment.py` (the award price converted at the ECB rate for the event's own date); `src/engine/fifo_manager.py` (`restate_stock_award_lot_on_vesting`, applied by the historical replay); `src/engine/event_processors/stock_award_processor.py` (the same methods for an award dated inside the tax year) | `tests/test_stock_award_lots.py` — `test_vesting_moves_no_shares_and_restates_the_cost`, `test_vesting_restates_only_its_own_award`; `tests/test_stock_award_scenarios.py` — `test_a_vesting_inside_the_tax_year_sets_the_cost_the_sale_is_measured_against`, which is red without the current-year processor | Valuation of a benefit in kind at the ueblicher Endpreis (§ 8 Abs. 2 Satz 1) on the day of Zufluss (§ 11 Abs. 1 Satz 1). **Zufluss is the booking.** BFH VI R 37/09 Rn. 4 holds that a Sperr- or Haltefrist does not prevent it and Leitsatz 2 that what does is a disposal being *rechtlich unmoeglich*, so the engine dates the acquisition on `AwardDate` and a later vesting has no ledger effect at all. **Superseded reading, 2026-08-13:** the first implementation restated the lot at vesting and was recorded as a choice under uncertainty; the research run at Q17 found the authority pointing the other way and the taxpayer chose the booking. **The factual condition the claim rests on:** the citation is Tier 4 and is decided under § 19 EStG, and it exempts only an *obligatorisch* restraint — so this position holds while the recipient could in fact have disposed of the shares before the condition lapsed. If a particular award makes disposal legally impossible until vesting, Leitsatz 2 puts Zufluss at the lapse and this row is wrong for it. **Measured on a real export, VZ 2025:** with the report read, both checkpoint marks keep the reconstruction's real lots instead of discarding them, and zero fallback lots are synthesised where there were two. **Residual uncertainty, recorded rather than resolved:** no column states which day `Price` was struck on, so where `ReportDate` and `VestingDate` differ the ueblicher Endpreis used may be either day's. It is the broker's measurement and is used as given; the alternative is to invent one from market data this engine does not hold. |
| GT-ESTG20-065 | **implements** | `src/engine/fifo_manager.py` — the restated lot's `unit_cost_basis_eur` is what a later disposal is measured against | `tests/test_stock_award_lots.py::test_the_whole_sequence_leaves_the_broker_s_quantity_and_the_vested_cost`, which asserts the cost follows the vestings while the quantity follows the awards | The value brought to tax on receipt is the Anschaffungskosten of the securities on a later disposal under § 20 Abs. 2 Satz 1 Nr. 1. This is the claim the 2025 declared figure turns on: without it the disposal has no basis at all. **Open question Q16** — whether a receipt the Freigrenze left untaxed still supplies Anschaffungskosten at full value — is not reached here, because the engine does not apply the Freigrenze (GT-ESTG23-009). |
...
| GT-ESTG20-014 | implements | `src/parsers/transfers_parser.py` + `RawTransferRecord` (reading the export), `DomainEventFactory.create_events_from_transfers` (collapsing its rows into one move each), `src/engine/event_processors/transfer_processor.py` (`apply_internal_transfer`, called by the historical replay and by the tax-year dispatch), `FifoLedger.receive_all_lots_from_transfer` | `test_internal_transfers.py`, `test_transfers_parser.py` | **Closed 2026-08-12.** The lots relocate between the two accounts' queues carrying acquisition date, cost basis, accumulated Vorabpauschale and `acquisition_date_is_known`; the lot objects are not rebuilt, so nothing can be lost in the copy. No `RealizedGainLoss` is produced — the move declares nothing. **What is not covered, and refuses rather than guesses:** a move of PART of a position, because `TransferPrice` is zero on every row of the standard export and nothing else says which lots moved (`INTERNAL_TRANSFER_PARTIAL`, FAIL_FAST); a transfer whose `Type` is not `INTERNAL`; and a transfer naming an account the input reports nowhere else (`TRANSFER_COUNTERPARTY_UNKNOWN`, FAIL_FAST) — the claim reaches the taxpayer's **own** depots, and IBKR's `INTERNAL` means the counterparty is an IBKR account rather than that it is theirs, so the ownership test is made from the input instead. **What the claim rests on being complete:** a Transfers export covering every year the engine replays. A year not exported is a year in which a move is invisible, so a window with a hole stops the run naming the missing years (`TRANSFERS_WINDOW_INCOMPLETE`, FAIL_FAST) — the export existing for other years means the query exists and the year is simply absent. An export absent for every year stays a warning: that is the ordinary state of anyone holding one account or having never moved anything. Cash rows are read and produce nothing here, because a currency move is not this claim's subject: [GT-ESTG20-014] is about securities, which relocate. **A currency balance does the opposite** — each account's is its own Kapitalforderung and a move between two of them is a disposal plus an acquisition, which the store now answers at [GT-FX-009] and values at [GT-FX-010]. Those two carry the cash rows; this claim reaches neither. |
...
person-level check would have passed. Red-first: the 13 lot-selection scenarios in
`test_per_account_fifo.py` **as that change wrote them** give 9 failed / 4 passed against
`origin/main`'s `src/`, and all 13 pass with it. The measurement is against that tree and not the
final one, and it cannot be repeated against the final one: the harness later grew a Transfers
writer, so the test file no longer imports against `origin/main` at all. The number was written
here as "measured on the final tree" until a review caught it.
...
**What this did not cover, and what closed it — measured on real data 2026-08-12.** Per-Depot lot
tracking needs to know where the units are, and a move between the taxpayer's own accounts was
invisible to the engine, because nothing read the Transfers export. Under per-account ledgers such
a move read as a loss in one account and an unexplained gain in the other at the next checkpoint,
so the reconstruction was discarded on both sides and replaced by lots carrying the broker's
quantity and an invented acquisition date. **The end-of-year reconciliation passed** — it compares
quantities, and the quantities were the broker's — so nothing downstream objected.

**The engine was not protecting anyone there, and an earlier wording of this block claimed it
was.** On the export measured the run did stop, but only because one holding is an Investmentfonds
and § 18 Abs. 2 refuses an invented month. Patching that one refusal showed what was behind it:
the run completed and **eight** declared figures moved against `origin/main`, five of them off
re-dated lots. A portfolio with no fund would have declared them without stopping. The refusal was
a property of the fund path, not of this change.

The effect reached every assessment year from the move onwards, not only the year of the move: the
historical replay spans the whole window, so a transfer in an early year re-dates lots that later
years still hold.

**Closed 2026-08-12 by the change that reads the Transfers export and relocates lots between
account queues ([GT-ESTG20-014] below).** On the same real export, a VZ 2025 run now completes
rather than aborting; every move is applied, both accounts reconcile against the broker without a
synthesised lot, and the eight moved figures reduce to the three the per-account snapshot
summation accounts for. Figures and per-instrument detail are in `VALIDATION_REPORT.md`.
...
| GT-FX-009 | implements | `src/engine/calculation_engine.py` — `currency_fifo_ledgers` keyed `(account, asset_id)` with the real account, `currency_ledger_accounts` deciding which exist, `_ensure_currency_ledger_exists`, `_reconcile_currency_soy` against that account's own opening balance, and the per-account currency EoY check; `src/parsers/parsing_orchestrator.py` — `cash_balances` recorded per `(account, currency)`; the currency lookup in every event processor | `test_per_account_currency.py`, `test_group7_currency_fifo.py`, `test_data_gap_channel_guards.py` | Each account's balance is its own Kapitalforderung, so a disposal consumes what was paid into that account and the end-of-year check runs per account — being too high in one and too low in another cancels out in the person's total. The person-level figures on `CashBalance` remain, as the sum, and no ledger is built from them. **What it rests on:** the cash report naming every account, which is the same thing every other per-account figure rests on. **Not covered:** a currency held in an account the cash report omits gets a ledger only from its own events, and its opening balance is then unknown rather than zero — **and its closing balance is compared against nothing**, where the pooled ledger it used to sit in was compared against the person's total. **This is not rare.** The pre-existing sub-0.01 threshold in `_process_cash_balance_positions` drops a row whose both ends are dust, and on the exports measured that leaves most of the currency ledgers with nothing to reconcile against and nothing to check — the counts, and the one non-dust residue among them, are in `VALIDATION_REPORT.md`. The condition predates this change; what this change does is make it per account, so the same holes are now per (account, currency) rather than per currency. A total-level backstop, or lowering the threshold, is follow-up work and is not attempted here. |
| GT-FX-010 | **choice under uncertainty** | `InternalCashTransferEvent` (`src/domain/events.py`), built from the export's `CASH` rows by `DomainEventFactory.create_events_from_transfers`; `InternalCashTransferProcessor` (`src/engine/event_processors/transfer_processor.py`) for the tax year and the `InternalCashTransferEvent` branch of `_apply_historical_currency_event` for earlier years | `test_per_account_currency.py::TestMovingMoneyBetweenYourAccounts`, `::TestAMoveInAnEarlierYear`, `::TestWhatACashMoveDoesNotDo`, `test_transfers_parser.py::TestCollapsingRowsIntoMoves` | **Reading A of Q15 is taken**: the Veräußerungspreis and the new lot's Anschaffungskosten are both the gemeiner Wert of the Kapitalforderung received — the amount moved, converted on the day of the move per [GT-ESTG20-022]. Reason: Rz. 131 ¶2 makes the Umbuchung a Veräußerung in terms, so Reading B contradicts a Tier 2 sentence, which CLAUDE.md's grey-area rule bars whichever way it falls; and Rn. 65/66, 66a and 69 measure a non-money consideration at the value of what is received in every configuration the administration addresses. **It is a selection, not compliance**, which is why this row is not `implements`: no located source names this case, and the claim sits in the store's *Open questions* section stating both readings rather than asserting one. **Not covered, and refuses:** a move naming an account the input reports nowhere else (`TRANSFER_COUNTERPARTY_UNKNOWN`, FAIL_FAST) — [GT-FX-009] reaches the taxpayer's own accounts and IBKR's `INTERNAL` does not establish that; a cash row whose `CashTransfer` is zero or blank; a move whose day has no exchange rate, on either path. A move of EUR is read and produces nothing. **Assumed, not guarded:** that no move names one account on both sides. |
