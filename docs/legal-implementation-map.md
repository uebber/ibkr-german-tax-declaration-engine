...
| GT-ESTG20-061 | **deviates** | `src/parsers/parsing_orchestrator.py` — `process_positions`, `_process_cash_balance_positions` | — (none yet) | The taxable subject is the person (§ 2 Abs. 1 S. 1 Nr. 5, § 25 Abs. 1 EStG), so a holding the broker reports per account is the **sum** across that person's accounts. The four snapshot loops assign rather than accumulate, so where one instrument or currency appears in two accounts the last row wins and the rest are discarded — quantity, cost basis and position value alike. Measured in `data_import/` on 2026-08-11: cash 5, 5 and 6 colliding currency keys in 2023/2024/2025; positions 0 in all four snapshots, so the securities half is latent. **Closed by the `fix-func(parsers)` that sums the reported snapshots across accounts.** Independent of Q2: this is whose holding, not which lot. |
