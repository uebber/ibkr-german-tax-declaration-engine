...
but every ledger key is written with a single `DEFAULT_ACCOUNT` constant, and no account identifier reaches a ledger, a lot or a disposal (the snapshot registries in `src/domain/assets.py` are keyed by account, which is what [GT-ESTG20-061] needed and this claim does not yet use)
