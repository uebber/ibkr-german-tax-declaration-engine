...
| GT-ESTG20-014 | **deviates** | — | — | Own-account transfer is not a disposal, and lots must relocate rather than close and reopen. Now that ledgers are per account ([GT-ESTG20-013]) this is reachable and unreached: the Transfers export is not read, so after a move the receiving account holds units it never acquired — the run stops at reconciliation or, before current-year securities disposals, at `SECURITIES_ACQUISITION_HISTORY_UNKNOWN` when surviving lots have unobserved acquisition history. Snapshot-only quantities cannot authorize a disposal. Supplied transfers will be supported by #88. Pooled, this was invisible; per account it is stated in every multi-account run (`MULTI_ACCOUNT_LIMITATIONS`). See the [GT-ESTG20-013] block above. |
...
at reconciliation or refuses a current-year disposal requiring undated reconstructed lots
(`SECURITIES_ACQUISITION_HISTORY_UNKNOWN`). Per-account tracking makes this visible (pooled, it was invisible and harmless), and
