...
| GT-ESTG20-064 | **choice under uncertainty** — Reading A of Q17 (Zufluss at the lapse of the condition) | `src/parsers/domain_event_factory.py` (`create_events_from_grants` — a vesting is dated on `VestingDate`, not the broker's `ReportDate`); `src/processing/enrichment.py` (the award price converted at the ECB rate for the event's own date); `src/engine/fifo_manager.py` (`restate_stock_award_lot_on_vesting`, applied by the historical replay); `src/engine/event_processors/stock_award_processor.py` (the same methods for an award dated inside the tax year) | `tests/test_stock_award_lots.py` — `test_vesting_moves_no_shares_and_restates_the_cost`, `test_vesting_restates_only_its_own_award`; `tests/test_stock_award_scenarios.py` — `test_a_vesting_inside_the_tax_year_sets_the_cost_the_sale_is_measured_against`, which is red without the current-year processor | Valuation of a benefit in kind at the ueblicher Endpreis (§ 8 Abs. 2 Satz 1) on the day of Zufluss (§ 11 Abs. 1 Satz 1), and Zufluss falling on the lapse of a condition under which the grantor may take the benefit back rather than on the booking. **The export corroborates the condition being real**, measured 2026-08-13: it carries a distinct activity kind reversing an earlier award, and the broker removes the basis at the original award price rather than the reversal-day price. **Measured on a real export, VZ 2025:** with the report read, both checkpoint marks keep the reconstruction's real lots instead of discarding them, and zero fallback lots are synthesised where there were two. **Residual uncertainty, recorded rather than resolved:** no column states which day `Price` was struck on, so where `ReportDate` and `VestingDate` differ the ueblicher Endpreis used may be either day's. It is the broker's measurement and is used as given; the alternative is to invent one from market data this engine does not hold. |
