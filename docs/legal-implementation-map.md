...
| GT-FX-010 | implements | `InternalCashTransferEvent` (`src/domain/events.py`), built from the export's `CASH` rows by `DomainEventFactory.create_events_from_transfers`; `InternalCashTransferProcessor` (`src/engine/event_processors/transfer_processor.py`) for the tax year and the `InternalCashTransferEvent` branch of `_apply_historical_currency_event` for earlier years | `test_per_account_currency.py::TestMovingMoneyBetweenYourAccounts`, `::TestWhatACashMoveDoesNotDo`, `test_transfers_parser.py::TestCollapsingRowsIntoMoves` | **Reading A of Q15 is the position taken** — the Veräußerungspreis and the new lot's Anschaffungskosten are both the gemeiner Wert of the Kapitalforderung received, i.e. the amount moved, converted on the day of the move per [GT-ESTG20-022]. Reason: Rz. 131 ¶2 makes the Umbuchung a Veräußerung in terms, so Reading B contradicts a Tier 2 sentence, which CLAUDE.md's grey-area rule bars whichever way it falls; and Rn. 65/66, 66a and 69 measure a non-money consideration at the value of what is received in every configuration the administration addresses. **It is an extension, not an application** — none of those Randziffern names a currency account, which is why Q15 stays open rather than being retired, and why this row says so rather than reading as settled law. **Not covered, and refuses:** a move naming an account the input reports nowhere else (`TRANSFER_COUNTERPARTY_UNKNOWN`, FAIL_FAST) — [GT-FX-009] reaches the taxpayer's own accounts and IBKR's `INTERNAL` does not establish that; a cash row whose `CashTransfer` is zero or blank, since that is the only column carrying the amount. A move of EUR is read and produces nothing. |
