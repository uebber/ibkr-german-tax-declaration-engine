...
| GT-ESTG20-061 | implements | `src/parsers/parsing_orchestrator.py` — `_fold_positions_by_asset` feeding `process_positions`; `_process_cash_balance_positions` | `test_multi_account_aggregation.py` | The taxable subject is the person (§ 2 Abs. 1 S. 1 Nr. 5, § 25 Abs. 1 EStG), so a holding the broker reports per account is the **sum** across that person's accounts. All five snapshot loops (SoY, EoY, and the three prior-year Vorabpauschale snapshots — start, end and opening) fold by resolved asset before writing, and the checkpoint-mark loop accumulates quantity and cost basis together so a mark's two figures describe the same holding. Additive fields sum. Per-unit fields (mark price, currency, multiplier) come from the first row, which is **arbitrary and deliberately left so**. A price per unit is not additive, and downstream it multiplies the summed unit count (`calculation_engine.py:2011`, `:2015`), so the only self-consistent combined figure would be summed value ÷ summed quantity. It cannot arise here — no instrument appears on more than one row in any of the four `Positions-*.csv` (measured 2026-08-11) — and a price per unit is a property of the fund on a date, repeated on every row rather than divided between them. The per-Depot work keeps each account's row intact, so nothing is combined and the choice disappears rather than being answered. No test pins it. The cash tiny-balance filter applies to the folded figures but keeps a currency whose accounts net to ~0 while any single balance is significant; thresholding the sum alone would delete an actively-held FX ledger. `BASE_SUMMARY` and `EUR` are excluded before the fold. **Independent of Q2** — this is whose holding, not which lot; FIFO remains pooled, see GT-ESTG20-013. `ClientAccountID` is deliberately still unmapped on `RawPositionRecord` (issue #69): aggregation folds by asset identity and never reads it, so mapping it here would be code no production path reaches. It becomes a prerequisite for the per-Depot flip. |
