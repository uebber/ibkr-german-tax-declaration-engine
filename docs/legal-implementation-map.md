...
| GT-ESTG20-014 | **deviates** | — | — | Own-account transfer is not a disposal, and lots must relocate rather than close and reopen. Now that ledgers are per account ([GT-ESTG20-013]) this is reachable and unreached: the Transfers export is not read, so after a move the receiving account holds units it never acquired — the run stops at reconciliation, or carries a synthesised basis where a later disposal restores the reported quantity. Pooled, this was invisible; per account it is stated in every multi-account run (`MULTI_ACCOUNT_LIMITATIONS`). See the [GT-ESTG20-013] block above. |
...
**Not yet covered — a move between the taxpayer's own accounts.** The Transfers export is not read, so
after an own-account transfer the receiving account holds units it never acquired: the run either stops
at reconciliation, or — where a later disposal happens to restore the reported quantity — carries a
synthesised basis. Per-account tracking makes this visible (pooled, it was invisible and harmless), and
every multi-account run states it in the report (`MULTI_ACCOUNT_LIMITATIONS`, a WARNING whose text says
the securities figures are not reliable if any position was ever moved). Closing it is
[GT-ESTG20-014].
