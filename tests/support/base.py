...
class FifoTestCaseBase:
    ...
    def _run_pipeline(self,
        ...
        # Transfers: written and passed ONLY when the scenario supplies rows, the same
        # way the prior-year snapshots are. A scenario that moves nothing between
        # accounts is left byte-identical to what it was before this file could write
        # the export at all.
        transfers_path = None
        if transfers_data is not None:
            transfers_path = os.path.join(os.path.dirname(paths["trades"]), "transfers.csv")
