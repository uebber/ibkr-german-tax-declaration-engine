...
class TestAdversarial(FifoTestCaseBase):
    ...
    def test_the_historical_replay_gives_each_account_only_its_own_past(self):
        ...
        by account too. Both accounts hold the SAME quantity (100), so the opening
        reconciliation cannot trim a ledger back to a different figure and repair a
        wrong split by accident -- the defect cannot hide behind a quantity gap.

        A: 100 @ 10 in 2023.  B: 100 @ 40 in 2024.  B sells 50 @ 35 in 2025.

        The seller is B, the holder of the NEWER lot, which is what makes the two
        readings diverge (a sale by A -- the older-lot holder -- would agree under
        both, because pooled FIFO takes the oldest lot and that is A's):

            per Depot : 1750 - 2000 = -250 LOSS,  acquired 2024-05-01 (B's own lot)
            pooled    : 1750 -  500 = +1250 GAIN, acquired 2023-05-01 (A's older
                        lot, which B never held)

        Sign, form line and acquisition date all differ, and the holding period
        with them.
        """
        isin = "US000000HR01"
        out = self._run_pipeline(
            trades_data=[
                ...
                trade_row(B, isin, "2025-06-01", "-50", "35", "SELL", "C", "T3"),
            ],
            positions_start_data=[
        ...
        assert len(rgl) == 1
        assert rgl[0].total_cost_basis_eur == Decimal("2000"), \
            "B's own 2024 lot, not A's older 2023 lot the pooled replay would hand it"
        assert rgl[0].gross_gain_loss_eur == Decimal("-250")
        assert rgl[0].acquisition_date == "2024-05-01"
    ...
    def test_an_account_that_only_reports_a_holding_at_year_end_is_checked(self):
        """The end-of-year check runs over every account with a ledger OR a closing
        row, so an account that appears only in the closing snapshot -- units it
        never acquired, with no ledger of its own -- is still checked and fails.

        A holds 100 all year and reconciles cleanly, so A raises nothing. B has no
        opening, no trade and no mark, so B has no ledger; the broker nonetheless
        reports 50 for B at year end. Checking only the accounts that have a ledger
        would skip B and accept 50 units from nowhere; the union with the closing
        rows is what catches it. A's clean reconciliation is deliberate, so B is the
        ...
        with pytest.raises(DataGapError, match="EOY_RECONCILIATION_FAILED"):
            self._run_pipeline(
                ...
                positions_end_data=[
                    position_row(A, isin, "100", "1000", price="10"),
                    position_row(B, isin, "50", "500", price="10"),
                ],
                custom_rate_provider=MockECBExchangeRateProvider(Decimal("1.00")),
                tax_year=TAX_YEAR,
            )
...
class TestTheLimitationsAreStated(FifoTestCaseBase):
    ...
    def test_two_accounts_are_told_what_is_not_covered(self):
        ...
        # engine has yet to fix: BMF Rz. 131 sets no Depot boundary on the FIFO of like
        # foreign-currency amounts, so per-person is a lawful reading and the open question is
        # decided separately (PR-D). Asserting a shortfall that may not exist in a
        # taxpayer-facing report is exactly what must not happen.
