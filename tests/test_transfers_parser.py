...
class TestReadingTheFile:
    ...
    def test_a_lot_detail_row_is_read_and_marked_by_its_empty_transaction_id(self, tmp_path):
        recs = parse_transfers_csv(_write(tmp_path, [
            transfer_row(A, B, "OUT", "20230601", isin=ISIN, quantity="-60", code="ST"),
        ]))
        # Blank arrives as an empty string, not None -- there is no validator on this
        # field. The factory tests both, which is why it strips before deciding.
...
class TestCollapsingRowsIntoMoves:
    ...
    def test_the_lot_detail_rows_are_dropped(self, tmp_path):
        """They describe the same units a second time. Summing every row would move
        the holding twice over."""
        moves = _moves(tmp_path, [
            transfer_row(A, B, "OUT", "20230601", isin=ISIN, quantity="-100", tx_id="X1"),
            transfer_row(A, B, "OUT", "20230601", isin=ISIN, quantity="-60", code="ST"),
            transfer_row(A, B, "OUT", "20230601", isin=ISIN, quantity="-40", code="ST"),
        ])
        assert len(moves) == 1 and moves[0].quantity == Decimal("100")
